...
(defn remove-peg
  "Take the peg at given postition out of the board"
  [board pos]
  (assoc-in board [pos :pegged] false))

(defn place-peg
  "Put a peg in the board at given postion"
  [board pos]
...
(defn can-move?
  ...
  [board]
  (some (comp not-empty (partial valid-move board))
        (map first (filter #(get (second %) :pegged) board))))

;; (filter #(get (second %) :pegged) {1 {:pegged true, :connections {6 3, 4 2}}})
